...
export class OrderPageComponent implements OnInit {
	...
	constructor(
		...
		this.momoErrorMessage$ = this.firestore.collection('messages').valueChanges();
		this.momoErrorMessage$.subscribe((res) => {
			for (let i = 0; i < res.length; i++) {
				if (res[i].type === 'momo-error' && res[i].message !== '' && res[i].message !== null) {
	...
	onSubmit(): void {
		...
		if ((this.invalidLocation || this.f['location'].errors) && this.showLocation) {
			this.isValidLocationOrPacks = true;
		...
		this.orderDetails = {
			...
			completed: false,
			location:
				this.orderForm.value.deliveryType === 'pick-up' ? 'Pick Up' : this.orderForm.value.location,
			deliveryType: this.orderForm.value.deliveryType,
			deliveryFee: this.orderForm.value.deliveryType === 'pick-up' ? 0 : this.deliveryFee,
		...
		this.http.post<PaymentResponse>(this.url, body, httpOptions).subscribe(
			(res: any) => {
				...
				this.payStackUrl = this.domSanitizer.bypassSecurityTrustResourceUrl(res.auth_url);
	...
	getPhoneNetWork(phoneNumber: string): string | null {
		...
		else if (networkDeterminants == '6' || networkDeterminants == '7') return 'AIRTELTIGO';
	...
	onCalculateFee(event: any): void {
		...
		const city: { name: string; price: number } | undefined = this.locations.find(
	...
	addAnotherItem() {
		...
		this.foodArray = this.socketService
			.getAllFoods()
			.filter((i) => !foodOrderedIds.includes(i.id) && i.category === this.category);
	...
	onSearchFood(searchTerm: any) {
		this.foodArray = this.socketService.onSearchFood(searchTerm.toLocaleLowerCase(), this.category);
	}
